...
class RecognitionResult:
    ...
    """Path to the annotated image on disk, if one was written."""
    ...
    def as_dict(self) -> dict:
        ...
        if self.annotated_path is not None:
            payload["annotated_path"] = self.annotated_path
        return payload
...
def build_summary(results: Sequence[RecognitionResult]) -> dict:
    """Build the JSON-serializable summary for a batch of recognition results.

    The returned dict has the shape::

        {
            "plates": ["ABC123", "DEF456", ...],
            "images": [<RecognitionResult.as_dict()>, ...]
        }

    ``plates`` is a flat list of all recognized plate texts (empty strings are
    dropped, duplicates preserved in the order they were seen). ``images``
    contains one entry per processed image with the full per-plate details.
    """
    all_texts: list[str] = []
    for r in results:
        for p in r.plates:
